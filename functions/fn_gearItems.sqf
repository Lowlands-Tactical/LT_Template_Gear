...
_period = param [0, 0, [0]];
_gear = param [1, "Test", ["Test"]];

_uniAmt = [5,5,2,2,2,2,1];
_trowAmt = [2,2,2,0,0,0];
_specAmt = [0,1,1,1,1,6,6];
_meds = ["ACE_fieldDressing","ACE_elasticBandage","ACE_quikclot","ACE_tourniquet","ACE_splint","ACE_morphine","ACE_epinephrine","ACE_bloodIV","ACE_plasmaIV_500","ACE_salineIV_500","ACE_surgicalKit"];
_medAmt = [50,50,10,12,15,30,20,6,6,2,1];
...
switch (_period) do 
{
	case 0:
	{
		...
		_role = ["ItemGPS","ACE_Flashlight_XL50","ACE_EntrenchingTool","ACE_IR_Strobe_Item","ACE_UAVBattery"];
		...
		_result = switch (_gear) do
		{
			...
			case "Role":		{_role};
			case "DMR":			{["ACE_RangeCard","ACE_SpottingScope", _radios #1, _role #0, _role #2]};
	...
	case 1:
	{
		_radios = ["ACRE_SEM52SL","ACRE_SEM70"];
		_role = ["ACE_DAGR","ACE_Flashlight_XL50","ACE_EntrenchingTool","ACE_IR_Strobe_Item",""];
	...
	case 2:
	{
		...
		_result = switch (_gear) do
		{
			...
			case "TrowAmt":		{_trowAmt};
			case "Role":		{_role};
			case "DMR":			{["ACE_RangeCard","", _radios #1, _role #0, _role #2]};
